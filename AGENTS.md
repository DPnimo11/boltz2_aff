...
## `peptide_systems/` — current Part-2 system set (2026-06-29)

`peptide_systems/` is the **active Part-2 data**, replacing the BH3/p53
peptide-ligand arm (which is retained under `papers/peptides/` and
`data/peptides/` but on hold — see "Planned Part 2"). It holds **13 classic
protein–protein complexes with experimental point-mutation ΔΔG data** (alanine
and non-alanine single mutants plus combinatorial variants): 1A22, 1AO7, 1BRS,
1CHO, 1GC1, 1JTG, 1VFB, 2B2X, 3BT1, 3HFM, 3S9D, 3SE3, 4G0N.

The file-name stem `<PDB>_<grp1>_<grp2>` names the two interacting chain groups
whose binding is measured (e.g. `1A22_A_B` = hGH chain A ↔ hGH-receptor chain B,
confirmed from the raw `.txt` partner columns). Per-system files:

- `<PDB>.pdb` — complex structure.
- `<PDB>.fasta` — chain sequences, regenerated by the `bat` csh helper via
  `pdb2fasta`.
- `<PDB>.mapping` — one line per residue, `RESNAME CHAIN PDBNUM SEQIDX`;
  resolves a mutation's `chain`+`resnum` to the PDB residue (and sequence index).
- `<PDB>_<g1>_<g2>.txt` — original raw mutational records (system, mutant Kd,
  WT Kd, partner names, mutation), superseded by `_New.txt`.
- `<PDB>_<g1>_<g2>_New.txt` — **curated table; the source of truth.** Schema below.
- `<PDB>_<g1>_<g2>_New.xlsx` — Excel mirror of `_New.txt`.
- `<PDB>_<g1>_<g2>.wt` — (some systems) the WT Kd sub-series as `Kd  count`
  lines (e.g. `3SE3_B_A.wt` → `2.50E-08 43` / `3.00E-08 24`: two WT references
  covering 43 and 24 variants).

### `_New.txt` column schema (canonical, example `1A22_A_B_New.txt`)

Tab-separated; header row, a `WT` reference row, one row per variant, then two
non-data footer lines. Columns:

1. `ID` — `<PDB>_<n>`; row 1 is the `WT` reference.
2. `Mutation` — substitution(s) `<wtAA><chain><resnum><mutAA>` (e.g. `HA18A` =
   His18 chain A → Ala). `WT` on the reference row. Multi-point variants are
   comma-joined and quoted (`"KA157A,EA163A"`). **Not only alanine scans** —
   non-Ala targets occur (`FB55S`, `RB11L`). Positions follow `.mapping`/PDB
   numbering.
3. `DG` — mutant binding ΔG (kcal/mol) **re-anchored to one global WT
   reference**: `DG = DG_WTref + (DGmut − DGWT)`. `DG_WTref` is the WT-row value
   (1A22 = −12.7160) and is repeated alone in the footer. Re-anchoring makes
   variants comparable across sub-series that have different absolute WT
   affinities.
4. `Activity_Mutate` — measured mutant Kd (M); `0` on the WT row.
5. `DGmut` — ΔG from the mutant Kd: `RT·ln(Kd)` ≈ `0.5925·ln(Kd)` at 298 K; `0`
   on the WT row.
6. `Activity_WT` — measured WT Kd (M) for **that mutation's sub-series** (a
   system can mix several WT references — 1A22 has 9.0e-10, 3.4e-10, 4.4e-10, …);
   `0` on the WT row.
7. `DGWT` — ΔG of that sub-series WT: `RT·ln(Activity_WT)`; `0` on the WT row.

Per-variant effect: **ΔΔG = `DGmut − DGWT`** (> 0 weakens binding, < 0
strengthens). Footer: (1) a lone value in the `DG` column = `DG_WTref`; (2)
`>1E-06  1.00E-06` = weak-binding detection cap (Kd above 1 µM reported at the
1 µM limit). `DG_WTref` differs slightly from each sub-series `DGWT` because it
is a single consensus anchor, not any one sub-series' WT.
...
> **On hold as of 2026-06-29.** The current Part-2 data is `peptide_systems/`
